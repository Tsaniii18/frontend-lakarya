...
interface ReimbursementDetail {
  ...
  approvals: Array<{ id: number; status: 'MENUNGGU' | 'DISETUJUI' | 'DITOLAK'; reviewNote: string | null; reviewedAt: string | null; approver: { name: string; department: { name: string } } }>;
}
...
async function cancelRequest() {
  ...
  errorMessage.value = '';
  try { const { data } = await api.patch<{ request: ReimbursementDetail }>(`/reimbursements/${request.value.id}/cancel`, {}, { headers: getAuthHeaders() }); request.value = data.request; successMessage.value = 'Pengajuan penggantian biaya berhasil dibatalkan.'; confirmOpen.value = false; }
  catch (error) { errorMessage.value = getApiErrorMessage(error); }
    ...
    <main class="min-w-0 px-5 py-7 sm:px-7 md:px-8 md:py-9 lg:px-10">
      ...
      <div v-else-if="request" class="mt-7 grid gap-5 lg:grid-cols-[minmax(0,1fr)_330px]">
        <section class="rounded-2xl border border-border bg-surface p-5 sm:p-6">
          ...
          <div v-if="request.status === 'MENUNGGU'" class="mt-7 flex justify-end border-t border-border pt-5"><button class="rounded-lg border border-danger px-4 py-2.5 text-sm font-semibold text-danger hover:bg-red-50" type="button" @click="confirmOpen = true">Batalkan Pengajuan</button></div>
        </section>
